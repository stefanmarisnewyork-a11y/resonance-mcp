...
class DB:
    ...
    def all_creatives(self):
        return [
            {**r, "scores": json.loads(r["scores_json"])}
            for r in self.query("SELECT * FROM creative_scores ORDER BY name")
        ]
...
TOOL_TIER = {
    ...
    "view_creatives": "internal_only",
    "creatives_by_motivation": "internal_only",
...
@mcp.tool()
def view_creatives() -> str:
    """[INTERNAL ONLY] List every scored creative in the database with its top
    motivations. The creative-side twin of view_program_profiles. INTERNAL because it
    enumerates all creatives across callers; external callers see only their own.
    """
    out = []
    for c in db.all_creatives():
        scores = {m: round(v * 100) for m, v in c["scores"].items() if v > 0}
        top = sorted(scores.items(), key=lambda kv: kv[1], reverse=True)[:4]
        out.append({"asset_id": c["asset_id"], "name": c["name"],
                    "source_db": c.get("source_db"),
                    "top_motivations": {m: s for m, s in top}})
    db.log("view_creatives", f"{len(out)} creatives")
    return json.dumps({"count": len(out), "creatives": out})


@mcp.tool()
def creatives_by_motivation(motivation: str, top_n: int = 20) -> str:
    """[INTERNAL ONLY] Rank stored creatives by their score on one motivation —
    e.g. 'all creatives high on Achievement'. The creative-side twin of
    programs_by_motivation. Accepts natural words and maps them to the 16. INTERNAL
    because it enumerates all creatives across callers.
    """
    SYN = {"family": "Belonging", "community": "Belonging", "togetherness": "Belonging",
           "romance": "Love", "relationships": "Love", "ambition": "Aspiration",
           "success": "Achievement", "fitness": "Fitness", "wealthy": "Wealth",
    ...
    if key not in MOTIVATIONS:
        ...
        if not mapped:
            return json.dumps({"error": f"'{motivation}' is not one of the 16 motivations and has no mapping",
                               "valid": MOTIVATIONS})
        key = mapped
    creatives = db.all_creatives()
    ranked = sorted(creatives, key=lambda c: c["scores"].get(key, 0), reverse=True)[:top_n]
    rows = [{"asset_id": c["asset_id"], "name": c["name"],
             "score_pct": round(c["scores"].get(key, 0) * 100)} for c in ranked]
    db.log("creatives_by_motivation", f"{motivation}->{key}, top {len(rows)}")
    return json.dumps({"motivation": key, "requested_as": motivation, "creatives": rows})
...
def preview_scored_creatives(scores_json: str) -> str:
    ...
    Optional per item: "name" — a readable display name (e.g. brand/tagline) stored
    alongside the resolved id, so the creative shows a friendly name AND keeps a stable id.
    ...
    for it in items:
        ...
        display_name = str(it.get("name", "")).strip() or res["name"]
        row = {"creative": label, **res, "display_name": display_name,
...
def commit_scored_creatives(scores_json: str, scored_by: str = "scorer") -> str:
    ...
    for it in items:
        ...
        raw = it.get("scores", {}) or {}
        res = _resolve_creative(label)
        # optional readable name: use it for display, keep the resolved stable id
        display_name = str(it.get("name", "")).strip() or res["name"]
        scores = {m: round(float(raw.get(m, 0)) / 100.0, 3) for m in MOTIVATIONS}
        db.store_graded_creative(res["creative_id"], display_name, scores)
        ...
        written.append({"creative_id": res["creative_id"], "name": display_name, "match": res["match"]})
...
def main():
    ...
    if transport == "http":
        ...
        # managed hosts (Render/DO) inject PORT and expect binding on 0.0.0.0
        host = os.environ.get("RESONANCE_HOST", "0.0.0.0")
        port = int(os.environ.get("PORT", os.environ.get("RESONANCE_PORT", "8848")))
        mcp.settings.host = host
        mcp.settings.port = port
        # Behind a managed-host proxy (DigitalOcean/Render), the forwarded Host header
        # is the public domain, which the default localhost-only allowlist rejects
        # ("Invalid Host header"). For this no-auth pilot, allow all hosts/origins.
        try:
